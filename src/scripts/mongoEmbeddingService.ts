...
interface DocumentMetadata {
  ...
  subcategory?: string;
  language?: string;
  docType?: 'conversation' | 'documentation';
  section?: string;
  source?: string;
  timestamp?: string;
}
...
interface EmbeddingProgress {
  ...
  lastProcessedConversation: string;
}

const delay = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));

export async function getProgress(): Promise<EmbeddingProgress> {
  try {
...
export async function updateProgress(filename: string): Promise<void> {
  const progressPath = path.resolve(__dirname, '../services/embedding_progress.json');
  await fs.writeFile(
    progressPath,
    JSON.stringify(
...
export async function readJsonFilesFromDirectory(directoryPath: string) {
...
async function processConversation(conversation: SupportDocument) {
...
export async function storeEmbedding(document?: Document) {
  try {
...
export async function processAndStoreJsonFiles(directoryPath: string) {
  try {
    ...
    for (const file of jsonFiles) {
      try {
        ...
        const supportDoc = file.content as SupportDocument;

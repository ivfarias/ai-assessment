...
const SUMMARY_SYSTEM_PROMPT = `You are a specialized AI focused on summarizing conversations. Your task is to:
...
export async function summarizeChatHistory(chatHistory: MemoryVariables): Promise<string> {
  ...
  const language = await detectLanguage(formattedHistory);

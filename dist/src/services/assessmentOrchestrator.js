...
export async function processAssessment(userId, db, assessmentName, input) {
    let user = await db.collection("user_profiles").findOne({ _id: userId });
    if (!user) {
        user = {
            _id: userId, updatedAt: new Date(), progress: { currentAssessment: null, stepIndex: 0, answers: {} }, profile: {}
        };
        await db.collection("user_profiles").insertOne(user);
    }
    ...
    await db.collection("user_profiles").updateOne({ _id: userId }, { $set: { "progress.currentAssessment": currentAssessmentName, "progress.stepIndex": currentStepIndex, "progress.answers": answers, updatedAt: new Date() } });
    ...
    const updatedUser = await db.collection("user_profiles").findOne({ _id: userId });
    let newScores;
    if (updatedUser) {
        newScores = scoringService.calculateOverallScore(updatedUser.profile);
        await db.collection("user_profiles").updateOne({ _id: userId }, { $set: { scoring: newScores, "progress.currentAssessment": null, "progress.stepIndex": 0, "progress.answers": {} } } // Reset progress
        );
    }
...
export async function startAssessmentByName(userId, assessmentName, db) {
...
//# sourceMappingURL=assessmentOrchestrator.js.map